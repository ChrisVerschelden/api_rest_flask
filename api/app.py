...
def get_pays(nom):
    """"Récupère les infos d'un pays en paramètre"""
...
def get_regions_for_pays(nom: str):
    """Récupère les regions d'un pays"""
    regions = execute_query("""select regions.nom, regions.code
                                    from regions
                                    join pays on regions.pays_id = pays.id
                                    where lower(pays.nom) = ?""", (urllib.parse.unquote(nom.lower()),))
    if regions == []:
        abort(404, "Aucune région dans ce pays")
    # ajout de _links à chaque dico region
    for i in range(len(regions)):
        regions[i]["_links"] = [{
            "href": "/regions/" + regions[i]["nom"],
            "rel": "self"
        }]
...
def post_regions_for_pays(nom_pays):
    """créé une region"""
    code_region = request.args.get("code")
    nom_region = request.args.get("nom")
    execute_query("insert into regions (code, nom, pays_id) values (?, ?, (select id from pays where nom = ?))", (code_region, nom_region, nom_pays))
    # on renvoi le lien du département  que l'on vient de créer
    reponse_json = jsonify({
        "_links": [{
            "href": "/regions/" + nom_region,
            "rel": "self"
        }]
    })
    return reponse_json, 201  # created
...
def get_ville_for_departement(nom: str):
    """Récupère les villes d'un departement"""
    ...
    if villes == []:
        abort(404, "Aucune ville dans ce departement")
    # ajout de _links à chaque dico ville
...
def post_ville_for_departement(nom_departement):
    ...
    execute_query("insert into villes (code, nom, departement_id) values (?, ?, (select id from departements where nom = ?))", (code_ville, nom_ville, nom_departement))
    # on renvoi le lien du département  que l'on vient de créer
    reponse_json = jsonify({
        "_links": [{
...
@app.route('/villes/<string:nom_ville>', methods=['DELETE'])
def delete_ville(nom_ville):
    """supprimer une ville"""
    execute_query("delete from villes where nom=?", (nom_ville, ))
    return "", 204
